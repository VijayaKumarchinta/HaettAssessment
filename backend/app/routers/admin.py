...
from app.services.discount_service import generate_discount_code
...
def approve_application(
    application_id: int, db: Session = Depends(get_db), admin=Depends(require_admin)
):
    application = (
        db.query(PartnerApplication)
        .filter(PartnerApplication.id == application_id)
    ...
    code = DiscountCode(
        ...
        code=generate_discount_code(application.business_name),
...
def summary(db: Session = Depends(get_db), admin=Depends(require_admin)):
    pending = (
        db.query(PartnerApplication)
        .filter(PartnerApplication.status == "PENDING")
        .count()
    )

    approved = (
        db.query(PartnerApplication)
        .filter(PartnerApplication.status == "APPROVED")
        .count()
    )

    rejected = (
        db.query(PartnerApplication)
        .filter(PartnerApplication.status == "REJECTED")
        .count()
    )

    total = db.query(PartnerApplication).count()
